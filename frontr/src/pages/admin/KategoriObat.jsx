...
const KategoriObat = () => {
  ...
  const fetchKategori = async () => {
    setIsLoading(true);
    try {
      const response = await api.get("/kategori-obat");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        // Jika sedang edit
        res = await api.put(`/kategori-obat/${editingId}`, formData);
        ...
        // Jika tambah baru
        res = await api.post("/kategori-obat", formData);
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        const res = await api.delete(`/kategori-obat/${id}`);
        if (res.data.success) {
          await Swal.fire({
            icon: "success",
